...
def definir_picons(url):
	...
	if not __useCustomPics__:
		canaisNaoEncontrados = 'start'
		fileListArray = []
		try:
			fileListCanais = urllib2.urlopen('https://prt01.000webhostapp.com/picons/lista.txt')
			for canalFile in fileListCanais:
				fileListArray.extend([canalFile.strip()])

		except urllib2.HTTPError, e:
			# print(e.code)
			urlErrorFound = 'n'

	canalEnviarLista = False
	...
	for linha in canais:
		...
		if not __useCustomPics__:
			# local
			canalFi = 'canal' + canalclean + '.png'

			canalFound = False
			if canalFi in fileListArray:
				canalFound = True

			if canalFound == False:
				if not canalFi.startswith( 'canalpg' ):
					canalEnviarLista = True
					canaisNaoEncontrados = canaisNaoEncontrados + '+' + canalFi

			newEnd = 'https://prt01.000webhostapp.com/picons/'
			url = newEnd + canalFi
	...
	# enviar lista
	if canalEnviarLista == True:
		import base64
		canaisPreEncoded = canaisNaoEncontrados.replace('start+','',1)
		canaisEncoded = base64.b64encode(canaisPreEncoded)
		# notifyLog('listencoded -->' + canaisEncoded + '<-->' + canaisNaoEncontrados)
		try:
			fileListCanais = urllib2.urlopen('https://prt01.000webhostapp.com/picons_script/add.php?addcanal=' + canaisEncoded)
		except urllib2.HTTPError, e:
			# print(e.code)
			urlErrorFound = 'n'
